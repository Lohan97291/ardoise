...
function ensureBoulardOrthographemicP1Sessions(days: Record<string, Session[]>): Record<string, Session[]> {
  ...
  for (const planned of BOULARD_ORTHOGRAPHEMIC_P1_SESSIONS) {
    ...
    const session: Session = {
      ...
      programmingItemId: `ortho-${planned.id}`,
      prepSheetId: planned.id,
      resourceId: planned.id,
      correctionMode: planned.id.endsWith("final") ? "dictation" : "none",
      note: "Fiche de préparation rattachée : consignes, parcours vert/or, matériel et photocopies. Utiliser « L’atelier des étiquettes » pour les temps de manipulation.",
    };
